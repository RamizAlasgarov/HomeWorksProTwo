...
Описание задачи:
Создать систему для хранения и обработки данных различных типов. Ваша система должна использовать дженерики,
 чтобы обеспечить безопасность типов и повторное использование кода.
 Также необходимо добавить обработку исключений для обеспечения надежности приложения.
...
Добавьте в класс Storage<T> метод для подсчета суммы всех чисел в контейнере,
 если T является числовым типом (например, Integer, Double и т.д.).
